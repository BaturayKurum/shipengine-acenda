...
import {
  AcendaBadRequestError,
  AcendaRequestError,
  AcendaUnauthorizedError,
} from './error';


// const PS_API_URL = process.env.PS_API_URL || 'https://shipstation.demo-niak.prestashop.net/api';
const PS_API_URL = 'http://192.168.33.10:8000/api/shipengine-connect';
...
function handleError(err: AxiosError<RequestError>) {
  logError(err);
  if (!err.response) {
    throw new AcendaRequestError(err.message);
  }

  const data = err.response?.data;
  if (err.response.status === 400) {
    throw new AcendaBadRequestError(data);
  }
  if (err.response.status === 401) {
    throw new AcendaUnauthorizedError(data);
  }
  if (err.response.status === 500) {
    throw new AcendaRequestError(data);
  }
}
...
export async function getOrders(req: GetOrdersRequest): Promise<any> {
  const { key, limit, filter } = req;
  ...
  console.log(params);
  const { data: res } = await client.request<any>({
    method: HttpMethod.POST,
    url: API_RESOURCE.Orders,
    // auth: getAuthData(req),
    headers: { api_key: key, Authorization: 'Bearer a7042a3dc6a8d5c30455bc580d808ef626d06e32' },
    params,
  });
